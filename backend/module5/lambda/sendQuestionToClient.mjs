...
const QUESTION_DATA_TABLE = process.env.Question_Data;
const PLAYER_ONLINE_TABLE = process.env.Player_Online_Table;
...
export const handler = async (event) => {
  ...
  let questionDataTableBody;

  // Fetch data from the Player_Online_Table
  const fetchDataFromPlayerConnected = await DYNAMO.send(
    new ScanCommand({ TableName: PLAYER_ONLINE_TABLE })
  );
  const playerEmailArray = fetchDataFromPlayerConnected.Items.map(
    (item) => item.playerEmail
  );

  // Fetch data from the Question_Data table
  questionDataTableBody = await DYNAMO.send(new ScanCommand({ TableName: QUESTION_DATA_TABLE }));
  let itemCount = questionDataTableBody.Count;
  questionDataTableBody = questionDataTableBody.Items;
  let quizTime = questionDataTableBody[0].quizTime;
...
*/
    ...
    return new Promise(async (resolve) => {
      ...
      for (const value of body) {
        ...
        await timeout(
          ...
          client,
          playersOnlinelist,
          dataFromGetSendDataRequestFromClient,
          gamePlayedTimes,
          playerEmailArray
        );
        if (i >= itemCount) {
  ...
  await sendEveryQuestion(10000, questionDataTableBody, client);
      ...
      try {
        const scoreListBody = await DYNAMO.send(
          new ScanCommand({
            TableName: PLAYER_SCORE_TABLE,
            FilterExpression:
              "#teamName = :teamName AND #quizName = :quizName AND #gamePlayedTimes = :gamePlayedTimes",
            ExpressionAttributeNames: {
              "#teamName": "teamName",
              "#quizName": "quizName",
      ...
      for (const playerConnectedId of playersOnlinelist) {
        const value1 = [
          value,
          {
            ...
            gamePlayedTimes: gamePlayedTimes,
            playerScoreList: playerScoreJson,
            playersOnline: playerEmailArray,
          },
        ];
        const requestParams = {
          ConnectionId: playerConnectedId.connectionId,
